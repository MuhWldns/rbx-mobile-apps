...
class AuthService {
  ...
  Future<bool> _oauthLogin(String baseEndpoint) async {
    ...
    debugPrint('[AuthService] opening OAuth URL: $url');
    try {
      final result = await _webAuth.authenticate(
        url: url,
        callbackUrlScheme: AppConstants.oauthCallbackScheme,
      );
      debugPrint('[AuthService] callback result: $result');
      final uri = Uri.parse(result);
      if (uri.queryParameters['error'] != null) {
        debugPrint('[AuthService] callback error: ${uri.queryParameters['error']}');
        return false;
      }
      final access = uri.queryParameters['access'];
      final refresh = uri.queryParameters['refresh'];
      if (access == null || refresh == null) {
        debugPrint('[AuthService] missing tokens in callback '
